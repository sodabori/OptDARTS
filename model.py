...
import random
from operations import FactorizedReduce, Identity, ResNetBasicblock, OPS, NAS_BENCH_201
...
class MixedOp(nn.Module):
  ...
  def __init__(self, C_in, C_out, stride, affine, track_running_stats, primitives):
    ...
    if args.auxiliary_skip:
      if self.stride == 2:
        self.auxiliary_op = FactorizedReduce(C_in, C_out, stride, affine, track_running_stats)
      elif args.auxiliary_operation == 'skip':
        self.auxiliary_op = Identity()
      elif args.auxiliary_operation == 'conv':
        self.auxiliary_op = nn.Conv2d(C_in, C_out, 1, padding=0, bias=False)
  ...
  def __init__(self, C_in, C_out, stride, max_nodes, op_names, affine=False, track_running_stats=True):
    ...
    for i in range(1, max_nodes):
      for j in range(i):
        node_str = '{:}<-{:}'.format(i, j)
        if j == 0:
          self.edges[ node_str ] = MixedOp(C_in, C_out, stride, affine, track_running_stats, op_names)
        else:
          self.edges[ node_str ] = MixedOp(C_in, C_out, 1, affine, track_running_stats, op_names)
...
class Network(nn.Module):
  ...
  def normalized_arch_parameters(self):
      with torch.no_grad():
  ...
  def new(self):
    model_new = Network(self._C, self._layerN, self.max_nodes, self._num_classes, self._criterion, search_space=NAS_BENCH_201, affine=False, track_running_stats=True).cuda() # network parameter 채워 넣기
